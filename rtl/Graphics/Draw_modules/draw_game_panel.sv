...
module draw_game_panel (
    ...
    bin_to_bcd3 u_selected_bcd (
        .binary(selected_number),
        .hundreds(selected_hundreds),
        .tens(selected_tens),
        .ones(selected_ones)
    );

    assign selected_ascii = {{4'h3, selected_hundreds},
                             {4'h3, selected_tens},
                             {4'h3, selected_ones}};
    ...
    always_comb begin
        ...
        if (summary_active) begin
            case (text_row)
                ...
                3'd3: begin
                    if (summary_multiplayer)
                        line_text = {"YOU      ", selected_ascii,
                                     " PTS", {12{8'h20}}};
                    else
                        line_text = {"SCORE    ", selected_ascii,
                                     " PTS", {12{8'h20}}};
                end
                3'd4: begin
                    if (summary_multiplayer) begin
                        line_text = {"RIVAL    ", selected_ascii,
                                     " PTS", {12{8'h20}}};
                    end else begin
                        case (summary_game_mode)
                            2'b00, 2'b10:
                                line_text = {"TARGET   ", selected_ascii,
                                             " SEC", {12{8'h20}}};
                            2'b01:
                                line_text = {"TARGET   ", selected_ascii,
                                             " PTS", {12{8'h20}}};
                ...
                3'd5: line_text = {"TIME     ", selected_ascii,
                                   " SEC", {12{8'h20}}};
                3'd6: line_text = {"LAST     ", selected_ascii,
                                   " SEC", {12{8'h20}}};
                default: line_text = {"BEST     ", selected_ascii,
                                      " SEC", {12{8'h20}}};
            endcase
        end else begin
            case (text_row)
                ...
                default: begin
                    if (uart_test_mode)
                        line_text = {"TX ", selected_ascii, "  RX ",
                                     selected_ascii, "  7SEG RX", {5{8'h20}}};
                    else
    ...
    always_comb begin
        ...
        if (in_text && (char_index < LINE_CHARS))
            current_char = line_text[((LINE_CHARS - 1 - char_index) * 8) +: 8];

        text_color = 4'h3;
        if (text_row == 3'd0) begin
            text_color = 4'h4;
        end else if (summary_active) begin
            if (text_row == 3'd1) begin
                case (match_result)
                    RESULT_WIN:  text_color = 4'h7;
                    RESULT_DRAW: text_color = 4'h4;
                    default:     text_color = 4'h5;
                endcase
            end else if ((text_row >= 3'd3) && (char_index >= 5'd9)) begin
                text_color = 4'h7;
            end
        end else begin
            if ((text_row == 3'd1) && (char_index >= 5'd9))
                text_color = 4'h7;
            else if ((text_row == 3'd2) && (char_index >= 5'd9))
                text_color = ((options_multiplayer && uart_peer_ready) ||
                              (uart_test_mode && uart_connected))
                           ? 4'h7
                           : ((options_multiplayer || uart_test_mode)
                              ? 4'h5 : 4'h3);
            else if (((text_row == 3'd3) || (text_row == 3'd4) ||
                      (text_row == 3'd5)) && (char_index >= 5'd9))
                text_color = 4'h7;
